    ...
    <div className="flex h-screen bg-slate-50 dark:bg-[#0c0a1a] transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-8 flex justify-between items-center">
              <div><h1 className="text-2xl font-bold text-slate-900 dark:text-white">Projetos Freelancer</h1><p className="text-slate-500 dark:text-white/40 text-sm mt-1">Gerencie projetos terceirizados</p></div>
              <button onClick={() => { setEditingProject(null); reset({ status: 'em_andamento' }); setIsModalOpen(true); }} className="bg-purple-600 hover:bg-purple-700 text-white rounded-xl shadow-sm shadow-purple-600/20 px-6 py-3 font-medium transition-all flex items-center gap-2"><Plus className="h-5 w-5" /> Novo Projeto</button>
          </div>
          ...
          <div className="bg-white dark:bg-white/[0.04] rounded-2xl shadow-sm border border-slate-200/80 dark:border-white/[0.06] p-6">
            ...
            <div className="flex items-center gap-4 mb-6">
               <div className="flex-1 relative"><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-slate-400" /><input type="text" placeholder="Buscar projetos..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-slate-300 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-transparent dark:text-white" /></div>
               <button onClick={() => setIsFilterModalOpen(true)} className="flex items-center gap-2 px-4 py-2 border border-slate-300 dark:border-white/10 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-slate-600 dark:text-slate-300"><Filter className="h-5 w-5" /> Filtros</button>
            </div>

            {loading ? ( <div className="text-center py-12"><Loader2 className="animate-spin h-10 w-10 text-purple-600 mx-auto" /></div> ) : filteredProjects.length === 0 ? ( <div className="text-center py-12"><div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 mb-4"><Briefcase className="h-8 w-8 text-slate-400" /></div><h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">Nenhum projeto</h3></div> ) : (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {filteredProjects.map((project) => (
                  <div key={project.id} className="bg-slate-50 dark:bg-[#0c0a1a] rounded-lg p-4 border border-slate-200 dark:border-white/[0.06] hover:shadow-md transition-all">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white dark:bg-[#1a1230] rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto border dark:border-white/[0.06] shadow-xl">
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div><label className="text-sm font-medium dark:text-slate-300">Nome do Projeto *</label><input {...register('name')} className="w-full px-3 py-2 border rounded-lg dark:bg-[#0c0a1a] dark:border-white/10" placeholder="Nome do projeto" /></div>
                <div><label className="text-sm font-medium dark:text-slate-300">Cliente *</label><select {...register('clientId')} className="w-full px-3 py-2 border rounded-lg dark:bg-[#0c0a1a] dark:border-white/10"><option value="">Selecione...</option>{clients.map(c => <option key={c.id} value={c.id}>{c.company ? c.company : c.name}</option>)}</select></div>
                <div><label className="text-sm font-medium dark:text-slate-300">Freelancer *</label><input {...register('freelancer')} className="w-full px-3 py-2 border rounded-lg dark:bg-[#0c0a1a] dark:border-white/10" placeholder="Nome" /></div>
                <div className="grid grid-cols-2 gap-4"><div><label className="text-sm font-medium dark:text-slate-300">Valor *</label><input {...register('value')} onChange={handleCurrencyInput} className="w-full px-3 py-2 border rounded-lg dark:bg-[#0c0a1a] dark:border-white/10" placeholder="R$ 0,00" /></div><div><label className="text-sm font-medium dark:text-slate-300">Prazo *</label><input {...register('deadline')} type="date" className="w-full px-3 py-2 border rounded-lg dark:bg-[#0c0a1a] dark:border-white/10" /></div></div>
                <div><label className="text-sm font-medium dark:text-slate-300">Status *</label><select {...register('status')} className="w-full px-3 py-2 border rounded-lg dark:bg-[#0c0a1a] dark:border-white/10"><option value="em_andamento">Em Andamento</option><option value="concluido">Concluído</option><option value="cancelado">Cancelado</option></select></div>
                <div><label className="text-sm font-medium dark:text-slate-300">Descrição</label><textarea {...register('description')} rows={3} className="w-full px-3 py-2 border rounded-lg dark:bg-[#0c0a1a] dark:border-white/10" /></div>
                <div><label className="text-sm font-medium dark:text-slate-300">Observações</label><textarea {...register('notes')} rows={2} className="w-full px-3 py-2 border rounded-lg dark:bg-[#0c0a1a] dark:border-white/10" /></div>
                <div className="flex gap-3 pt-4 border-t dark:border-white/[0.06]"><button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 px-4 py-2 border rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 dark:text-white">Cancelar</button><button type="submit" disabled={isSubmitting} className="flex-1 bg-purple-600 hover:bg-purple-700 text-white rounded-xl shadow-sm shadow-purple-600/20 px-4 py-2">{isSubmitting ? 'Salvando...' : 'Salvar'}</button></div>
              </form>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-md">
           <div className="bg-white dark:bg-[#1a1230] rounded-2xl max-w-sm w-full p-6 border dark:border-white/[0.06]">
            ...
            <div className="space-y-4">
                <div><label className="text-sm dark:text-slate-300">Status</label><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as ProjectStatusFilter)} className="w-full px-3 py-2 border rounded-lg dark:bg-[#0c0a1a]"><option value="all">Todos</option><option value="em_andamento">Em Andamento</option><option value="concluido">Concluído</option></select></div>
                <div><label className="text-sm dark:text-slate-300">Cliente</label><select value={clientFilter} onChange={(e) => setClientFilter(e.target.value)} className="w-full px-3 py-2 border rounded-lg dark:bg-[#0c0a1a]"><option value="all">Todos</option>{clients.map(c => <option key={c.id} value={c.id}>{c.company || c.name}</option>)}</select></div>
            </div>
            <div className="flex gap-3 mt-6"><button onClick={() => {setStatusFilter('all'); setClientFilter('all'); setIsFilterModalOpen(false);}} className="flex-1 border rounded-lg py-2 dark:text-white">Limpar</button><button onClick={() => setIsFilterModalOpen(false)} className="flex-1 bg-purple-600 hover:bg-purple-700 text-white rounded-xl shadow-sm shadow-purple-600/20 py-2">Aplicar</button></div>
          </div>
        </div>
      )}
    </div>
