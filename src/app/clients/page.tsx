...
export default function ClientsPage() {
      ...
      clients.forEach(client => {
        if (client.contractStartDate && client.contractDuration) {
          ...
          if (daysUntilExpiration <= 30 && daysUntilExpiration > 0) {
            // Usando toast para notificação
            toast({
              title: "Aviso de Contrato",
              description: `O contrato de ${client.name} expira em ${daysUntilExpiration} dias.`,
              variant: "default",
            });
          }
        }
  ...
  const addDocument = () => {
    if (!selectedClientForDocs || (!newDocument.url && !uploadedFile) || !newDocument.name) {
      toast({ title: "Erro", description: "Preencha o nome e forneça um arquivo ou URL.", variant: "destructive" });
      return;
    }
  ...
  const getContractStatus = (client: Client) => {
    if (!client.contractStartDate || !client.contractDuration) return { status: 'none', days: 0, color: 'bg-slate-100 text-slate-700' };
      ...
      {isFilterModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-md w-full">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold">Filtrar Clientes</h2>
                <button
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setContractFilter(e.target.value as ContractStatusFilter)}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
