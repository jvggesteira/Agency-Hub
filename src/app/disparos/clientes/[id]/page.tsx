...
export default function ClientDetailPage() {
  ...
  const [dispForm, setDispForm] = useState({ packageId: '', name: '', dispatchDate: '', sentMessages: '', deliveredMessages: '', redirectionCost: '', notes: '' });
  ...
  // ─── Dispatch Handlers ───────────────────────────────────────────────────────
  const resetDispForm = () => setDispForm({ packageId: '', name: '', dispatchDate: '', sentMessages: '', deliveredMessages: '', redirectionCost: '', notes: '' });
    ...
    try {
      if (dispEditId) {
        await updateDispatch(dispEditId, {
          name: dispForm.name, dispatch_date: new Date(dispForm.dispatchDate).toISOString(),
        ...
        await createDispatch({
          ...
          dispatch_date: new Date(dispForm.dispatchDate).toISOString(),
          sent_messages: parseInt(dispForm.sentMessages), delivered_messages: parseInt(dispForm.deliveredMessages),
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-4">
              <Button variant="ghost" size="sm" onClick={() => router.push('/disparos/clientes')}>
                            ...
                            <div className="flex items-start justify-between">
                              <div>
                                ...
                                <p className="text-xs text-slate-500 dark:text-white/40">
                                  {new Date(d.dispatch_date).toLocaleDateString('pt-BR')} · {pkg?.name || 'Pacote removido'}
                                </p>
                              </div>
                              <div className="flex gap-1">
                                ...
                                <button onClick={() => { setDispEditId(d.id); setDispForm({ packageId: String(d.package_id), name: d.name, dispatchDate: d.dispatch_date.split('T')[0], sentMessages: String(d.sent_messages), deliveredMessages: String(d.delivered_messages), redirectionCost: d.redirection_cost ? String(d.redirection_cost) : '', notes: d.notes || '' }); setDispOpen(true); }}
                                  className="p-1.5 rounded-md hover:bg-slate-100 dark:hover:bg-white/[0.06] text-slate-400 hover:text-slate-900 dark:hover:text-white" title="Editar">
                                  <Pencil className="h-3.5 w-3.5" />
                                </button>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 mt-2">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleDispSubmit} disabled={dispSaving || !dispForm.name || !dispForm.dispatchDate || !dispForm.sentMessages} className="bg-purple-600 hover:bg-purple-700 text-white">
                {dispSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : dispEditId ? 'Salvar' : 'Registrar'}
              </Button>
            </div>
          </div>
        </DialogContent>
